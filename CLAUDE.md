...
- Aktivitätsarten-Einstellungen: pro Aktivitätsart per Checkbox statt Freitext auswählbar, welche Intervals.icu-Rohtypen (Sportarten) automatisch dazu passen (`ICU_TYPE_LABELS` in app.js, vordefinierte Liste). Sportarten unterhalb im selben Reiter "Aktivitätsarten" vollständig verwaltbar: vordefinierte umbenennbar/ausblendbar (jederzeit über "Wieder anzeigen" reaktivierbar) und eigene neu anlegbar/endgültig löschbar (Rohtyp + Bezeichnung) — Overrides in eigener GitHub-Sync-Datei `icu-type-labels.json` (Muster wie `activity-kinds.json`). Für eine neue eigene Sportart muss der technische Rohtyp-Name (z.B. `AlpineSki`) bekannt sein *(Stand: 04.09.2026)*
